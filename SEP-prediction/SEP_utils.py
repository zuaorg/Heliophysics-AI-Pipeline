...
def save_prediction_results(datafile, e_type, time_window, y_true,y_pred, y_calibrated_prop,result_dir='results'):
    os.makedirs(result_dir,exist_ok=True)
    result = pd.read_csv(datafile)
    # Determine the output filename
    predictions_file = os.path.join(result_dir,
                                    'SEP_prediction_results_' + str(e_type) + '_' + str(time_window) + '.csv')
    print('Saving result to file:', predictions_file)
    # Convert y_pred values: 'P' if the value is 1, and 'N' if the value is 0.
    converted_preds = ['P' if pred == 1 else 'N' for pred in y_pred]
    # Insert the "Predicted SEP" column with y_pred values at position 4 (i.e. at index 3)
    result.insert(4, 'Predicted SEP', converted_preds)
