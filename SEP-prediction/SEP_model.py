...
class SEPModel:
    ...
    def build_base_model(self,
            ...
            model = layers.Dense(100, kernel_regularizer=l2(0.001))(input_layer)
            model = layers.Dense(100, kernel_regularizer=l2(0.001))(model)
            model = layers.Bidirectional(layers.LSTM(units=200,kernel_regularizer=l2(0.001), return_sequences=True, dropout=0.3, recurrent_dropout=0.2))(model)
    ...
    def compile(self,loss='binary_crossentropy',metrics=['accuracy'], adam_lr=0.0001):
    ...
    def save_model(self, e_type='fc', time_window=12, w_dir=None):
        e_type = str(e_type).lower().replace('_s', '')
        model_dir = 'models' + os.sep + 'sep_model_' + str(e_type) + '_' + str(time_window) + 'hr'

        if w_dir is not None:
            model_dir = w_dir + os.sep + 'sep_model_' + str(e_type) + '_' + str(time_window) + 'hr'

        if os.path.exists(model_dir):
            shutil.rmtree(model_dir)
        os.makedirs(model_dir)

        # Save both architecture and weights in a single H5 file
        model_path = model_dir + os.sep + 'model.h5'
        self.model.save(model_path)  # This saves both the architecture and weights
    ...
    #     self.model.save_weights(weight_dir + os.sep + 'model.weights.h5')
    ...
    def load_weights(self,e_type='fc',time_window=12,w_dir=None):
        ...
        if not os.path.exists(weight_dir):
            ...
            if not w_dir == 'default_models':
                ...
                exit()
        self.build_model(weight_dir + os.sep + 'model.weights.h5')
        if self.model == None :
            print('Error: You must train a model first before loading the weights.')
            exit()
        print('Loading weights from:', weight_dir + os.sep + 'model.weights.h5')
        self.model.load_weights(weight_dir + os.sep + 'model.weights.h5').expect_partial()
    ...
    def load_model(self,input_shape=(series_len,n_features),
        ...
        e_type = str(e_type).lower().replace('_s','')
        model_dir = w_dir + os.sep + 'sep_model_' + str(e_type) + '_' + str(time_window) + 'hr'
        model_path = model_dir + os.sep + 'model.h5'  # The H5 model file that includes both weights and architecture

        try:
            # Load the entire model (architecture + weights)
            print(f"Loading model from {model_path}")
            self.model = models.load_model(model_path, custom_objects={'SEPAttention': SEPAttention})

            # Compiling the model
            self.model.compile(loss=self.loss, metrics=self.metrics,
                               optimizer=tf.keras.optimizers.Adam(learning_rate=self.adam_lr))
    ...
    def build_model(self,w):
        ...
        self.input_shape = self.get_input_shape_from_h5(w)
        # Ensure input_shape is valid
        if self.input_shape is None:
            raise ValueError(f"Could not find input shape in the provided file: {w}")
