...
def train_model(e_type, start_hour, end_hour):
    ...
    for k in range(start_hour,end_hour,12):
        ...
        x_test, y_test, nb_test, columns = load_data(datafile= testing_data_file, cols_list=cols_list)
        ...
        for i in range(0,iterations):
            ...
            predictions_proba = expit(predictions_atten_proba)  # Convert logits to probabilities
            fop, mpv = calibration_curve(y_test, predictions_proba, n_bins=10)
            ir = IR()
            ...
            fop, mpv = calibration_curve(y_test, cal_pred, n_bins=10)
